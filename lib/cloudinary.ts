...
export const imagePresets = {
  // c_fit (not c_fill) — resizes to fit within the box without cropping, so
  // the full photo survives. Cards render it via object-contain + a neutral
  // background fill, letterboxing whatever's left instead of cutting the
  // image off.
  propertyCard: (url: string) => optimizeUrl(url, "w_600,h_400,c_fit,q_75,f_auto"),
  propertyHero: (url: string) => optimizeUrl(url, "w_1200,h_675,c_fill,q_80,f_auto"),
  blogCover: (url: string) => optimizeUrl(url, "w_800,h_450,c_fit,q_75,f_auto"),
